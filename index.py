def reading_source(path):
    ...
    checkWord6 = "초대했습니다"
    checkWord7 = "내보냈습니다"
    checkWord8 = "오" # 오전, 오후 -> '오'
    checkWord9 = "월" # 년월일
    checkWord10 = "년" # 년월일
    checkWord11 = "방장이 되었습니다."
    
   
    for idata in data :
        if (    (checkWord1 in idata) & (checkWord2 in idata) & (checkWord3 not in idata) 
        & (checkWord4 not in idata) & (checkWord5 not in idata) & (checkWord6 not in idata) 
        & (checkWord7 not in idata) &  (checkWord8 in idata) & (checkWord9 in idata)    
        & (checkWord10 in idata) & (checkWord11 not in idata)  ): 
            name = idata.split(', ')
            name = name[1][0:]
            name = name.split(':')
            name = name[0][0:]
